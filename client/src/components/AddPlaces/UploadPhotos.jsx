...
const URL_TO_UPLOADS2 = config.UPLOADS_URL;
  ...
  return (
    <>
      <div className="flex gap-4">
        <input
          type="text"
          placeholder="URL мекенжайын осында қойыңыз..."
        ...
        {uploadPhotos.length > 0 &&
          uploadPhotos.map((photo) => (
            <div className="flex relative" key={photo}>
              ...
              >
                {photo === uploadPhotos[0] && <AiFillStar size={26} />}
                {photo !== uploadPhotos[0] && <AiOutlineStar size={26} />}
              </button>
              <button
      ...
      <label className="flex h-32 mt-2 max-w-lg md:max-w-xs items-center justify-center cursor-pointer gap-1 border bg-transparent rounded-2xl text-gray-500">
        ...
        Құрылғыдан жүктеу
        <input onChange={uploadFromDevice} type="file" multiple hidden />
      </label>
    </>
  );
}
