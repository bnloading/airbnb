    ...
    return (
      <div>
        ...
        <div className="flex justify-center">
          <h1 className="text-2xl font-semibold mt-12">Әлі брондаулар жоқ.</h1>
        </div>
      </div>
    );
  }
              ...
              {place.place ? (
                <>
                  <div className="flex max-w-full h-46 md:max-w-[320px] md:h-44 bg-gray-300 shrink-0 rounded-tr-2xl rounded-tl-2xl md:rounded-tr-2xl md:rounded-tl-2xl md:rounded-2xl">
                    <img
                      ...
                      src={URL_TO_UPLOADS + place.place?.photos?.[0]}
                      alt=""
                    />
                  </div>
                  <div className="flex flex-col gap-4 px-4 pb-6 mt-3 text-sm md:text-base ">
                    ...
                    {user?.isAdmin && place.user && (
                      <div className="flex items-center gap-2 text-gray-600 bg-blue-50 rounded-lg px-3 py-2">
                        <FaRegUserCircle size={18} />
                        <span className="font-medium">{place.user.name}</span>
                    ...
                    <div className="flex flex-row items-center text-gray-500">
                      ...
                      <p className="mr-2">
                        {format(new Date(place.checkIn), "MMM-dd-yyyy")}
                      </p>
                      дейін
                      <BsCalendarDate className="ml-2 mr-1" />
                      <p>{format(new Date(place.checkOut), "MMM-dd-yyyy")}</p>
                    </div>
                    <div className="flex flex-row items-center gap-2">
                      <AiOutlineCreditCard size={26} />
                      <p className="font-semibold text-lg">
                        Жалпы баға: {place.price} ₸
                      </p>
                    </div>
                  </div>
                </>
                ...
                <div className="flex flex-col items-center mx-auto py-8 px-4">
                  ...
                  <p className="items-center text-center text-gray-500">
                    Иесі бұл орынды жойған болуы мүмкін. Кейінірек қайталап
                    көріңіз немесе бізге хабарласыңыз.
                  </p>
                </div>
              )}
            </Link>
