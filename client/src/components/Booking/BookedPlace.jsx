...
function BookedPlace() {
  ...
  return (
    <div className="max-w-global mx-auto">
      <div className="mt-12">
        <h1 className="text-2xl font-semibold">{bookedPlace.place.title}</h1>
        ...
        <div className="bg-gray-100 rounded-xl text-sm md:text-base px-2 md:px-6 py-4 mt-2">
          ...
          <div className="flex flex-row items-center text-gray-500">
            ...
            <p className="mr-2">
              {format(new Date(bookedPlace.checkIn), "MMM-dd-yyyy")}
            </p>
            дейін
            <BsCalendarDate className="ml-2 mr-1" />
            <p>{format(new Date(bookedPlace.checkOut), "MMM-dd-yyyy")}</p>
          </div>
          <div className="mt-4">
            <h6 className="text-base font-semibold">Брондаған:</h6>
            <p className="text-gray-500 text-base">{bookedPlace.name}</p>
          </div>
          <div className="mt-4">
            <h6 className="text-base font-semibold">Жалпы баға:</h6>
            <p className="text-2xl font-bold">{bookedPlace.price} ₸</p>
          </div>
        </div>
      </div>
          ...
          {bookedPlace.place.photos?.[0] && (
            <div>
              <img
                ...
                src={URL_TO_UPLOADS + bookedPlace.place.photos?.[0]}
                alt=""
              />
            </div>
          )}
          ...
          <div>
            {bookedPlace.place.photos?.[1] && (
              <div>
                <img
                  className="aspect-square object-cover rounded-2xl"
                  src={URL_TO_UPLOADS + bookedPlace.place.photos?.[1]}
                  alt=""
                />
              </div>
            )}
            ...
            {bookedPlace.place.photos?.[2] && (
              <div className="overflow-hidden">
                <img
                  ...
                  src={URL_TO_UPLOADS + bookedPlace.place.photos?.[2]}
                  alt=""
                />
              </div>
            )}
